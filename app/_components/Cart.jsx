  ...
  return (
    <div>
        ...
        <div className='mt-5 flex flex-col gap-3'>
            <h2 className='font-bold'>Mои заказы</h2>
            {cart&&cart.map((item,index)=>(
                <div key={index} className='flex justify-between gap-8 items-center'>
                    ...
                    <h2 className='font-bold flex gap-2'>{item.price} тенге
        ...
        {CalculateCartAmount()>0&&    <Button className="w-full">Оплатить {CalculateCartAmount()} тенге</Button>}
            </Link>
        </div>
    </div>
  )
